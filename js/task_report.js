...
const urlDate = urlParams.get('task_date') || new Date().toISOString().split('T')[0];
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = e => {
            ...
            if (!db.objectStoreNames.contains('tbl_finished_tasks')) {
                db.createObjectStore('tbl_finished_tasks', { keyPath: 'id' });
            }
        };
    });
}
...
async function getClientDetails(clientId) {
    ...
    return new Promise(resolve => {
        ...
        req.onsuccess = e => resolve(
            e.target.result.find(c => c.uryyToeSS4 === clientId) || null
        );
    });
}
...
function renderStatusButtons(selected = '') {
    ...
    for (const status in taskStatusOptions) {
        const color = taskStatusOptions[status];
...
async function renderSelectedActivity() {
    document.getElementById('selectedActivity').innerHTML =
        `${activityTitle} <span class="badge bg-info ms-2">Task</span>`;
    document.getElementById('activityDescription').textContent = activityDetails;

    const db = await openDB();
    const store = db
        .transaction('tbl_finished_tasks', 'readonly')
        .objectStore('tbl_finished_tasks');
...
/* ---------- Sync to MySQL ---------- */
async function syncTaskToServer(record) {
    try {
        const formData = new FormData();
        for (const key in record) formData.append(key, record[key]);

        const response = await fetch('sync_finished_tasks.php', {
            method: 'POST',
            body: formData
        });
        ...
        if (!result.success) {
            console.error('Failed to sync task:', result.message);
        }
    } catch (err) {
...
document.getElementById('activityReportForm').addEventListener('submit', async e => {
    ...
    const db = await openDB();
    const store = db
        .transaction('tbl_finished_tasks', 'readwrite')
        .objectStore('tbl_finished_tasks');
